...
export default function DataKontrakDetail(props) {
  ...
  const getData = async () => {
    ...
    const encodedQuery = encodeURIComponent(
      `SELECT a.id,a.id_dispensasi,a.thang,a.kdsatker,a.kdkppn,a.tgpermohonan, a.nopermohonan,a.nokontrak,a.tgkontrak,a.nilkontrak FROM  laporan_2023.dispensasi_kppn_lampiran a WHERE a.id_dispensasi='${props.id}' GROUP BY a.id ORDER BY id DESC`
    );
  ...
  const handleHapus = async (id, kdsatker, kdkppn, id_dispensasi) => {
    Swal.fire({
              ...
              {data.map((row, index) => (
                <tr key={index}>
                  ...
                  <td className="align-middle baris-total text-end">
                    {numeral(row.nilkontrak).format("0,0")}
                  </td>
                  {role !== "2" && (
