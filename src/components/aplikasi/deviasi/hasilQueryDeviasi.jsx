...
const HasilQueryDeviasi = (props) => {
  ...
  const getData = async () => {
    ...
    try {
      const response = await axiosJWT.get(
        import.meta.env.VITE_REACT_APP_LOCAL_INQUIRY_GETDATA_DEVIASI
          ? `${
              import.meta.env.VITE_REACT_APP_LOCAL_INQUIRY_GETDATA_DEVIASI
            }${encryptedQuery}&page=${page}&limit=${limit}&user=${username}&jenlap=${
              props.jenlap
            }`
          : "",
        {
      ...
      setTotalPagu(response.data.pagu);
  ...
  const columnsAll = Object.keys(data[0] || {});

  // Filter columns to exclude those with the name 'PAGU'
  const columns = columnsAll;

  let jumlahKolombaru = props.jenlap === "2" ? 24 : 25;
  const jumlahKolom = Object.keys(data[0] || {}).length;
  const columnTotals = new Array(jumlahKolombaru).fill(0);

  data.forEach((row) => {
    for (
      let cellIndex = jumlahKolom - jumlahKolombaru;
      cellIndex < jumlahKolom;
      cellIndex++
    ) {
      columnTotals[cellIndex - (jumlahKolom - jumlahKolombaru)] += Number(
  ...
  let baris = props.jenlap === "2" ? 25 : 26;
  let totalsubtotal = props.jenlap === "2" ? 23 : 24;
  let totalbaris = props.jenlap === "2" ? 24 : 24;
  return (
    <>
      <Modal
        onHide={tutupModal}
                  ...
                  <tfoot>
                    <tr>
                      <td
                        ...
                        colSpan={jumlahKolom - totalsubtotal}
                      >
                        SUB TOTAL
                      </td>
                    ...
                    <tr>
                      <td
                        ...
                        colSpan={jumlahKolom - totalbaris}
                      >
                        TOTAL
                      </td>
...
export default HasilQueryDeviasi;
