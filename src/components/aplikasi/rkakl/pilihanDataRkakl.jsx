...
export const SwitchBlokir = ({ blokir, onChange }) => {
  const [switchBlokir, setswitchBlokir] = useState(false);
  const [value, setValue] = useState("");

  useEffect(() => {
    setValue(blokir);
  }, [blokir]);
  const handleSwitchBlokir = () => {
    const newSwitchBlokir = !switchBlokir;
    setswitchBlokir(!switchBlokir);
    onChange(newSwitchBlokir);
    setswitchBlokir(!switchBlokir);
  };
  return (
    <>
      <Form.Check
        ...
        id="custom-switch"
        label="Kode Blokir"
        checked={switchBlokir}
        onChange={handleSwitchBlokir}
      />
    </>
  );
};
