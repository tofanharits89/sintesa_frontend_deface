...
import KdBulan25 from "../../referensi/referensi_dau25/ref_bulan";
import { CekAlokasi25 } from "../../referensi/referensi_dau25/cek_Alokasi";

const RekamKMKModalTambahPenundaan25 = ({ show, onHide, id }) => {
  ...
  const getData = async () => {
    ...
    const encodedQuery = encodeURIComponent(
      ...
      SUBSTR(a.uraian,1,40) uraian,a.jenis,a.kriteria,nm_kriteria,nmjenis,a.status_cabut FROM tkd25.ref_kmk_dau a
      LEFT OUTER JOIN ( SELECT b.jenis,b.nmjenis AS nmjenis FROM tkd25.ref_kmk b) b ON a.jenis=b.jenis
      LEFT OUTER JOIN ( SELECT c.id_kriteria,c.nm_kriteria AS nm_kriteria FROM tkd25.ref_kmk_dau_kriteria c) c 
  ...
  const handleSubmitdata = async (values, { setSubmitting }) => {
    ...
    try {
      //  console.log(values);
      await axiosJWT.post(
        import.meta.env.VITE_REACT_APP_SIMPANPENUNDAAN_25,
        values,
        {
          headers: {
            Authorization: `Bearer ${token}`,
...
export default RekamKMKModalTambahPenundaan25;
