...
const RekonDetail = ({
  ...
  const getData = async () => {
    ...
    const encodedQuery = encodeURIComponent(
      ...
      FROM TKD25.REKAP A 
      LEFT OUTER JOIN
      (SELECT B.THANG,B.KODE_PEMDA,B.PERIODE,B.KODE_KPPN,SUM(NILAI_PEMOTONGAN) POTONGANOMSPAN FROM DATA_OMSPAN25.DATA_PEMOTONGAN B
      ...
      LEFT OUTER JOIN
      (SELECT C.THANG,C.KODE_PEMDA,C.PERIODE,C.KODE_KPPN,SUM(NILAI_PENUNDAAN) PENUNDAANOMSPAN FROM DATA_OMSPAN25.DATA_PENUNDAAN C
    ...
    try {
      const response = await axiosJWT.get(
        import.meta.env.VITE_REACT_APP_GENERATEDATA_REKON_OMSPAN_25
          ? `${
              import.meta.env.VITE_REACT_APP_GENERATEDATA_REKON_OMSPAN_25
  ...
  const getDataPotongan = async () => {
    const encodedQuery = encodeURIComponent(
      `SELECT a.thang,a.bulan,a.kdkppn,a.kdpemda,a.akun_pusat,a.akun_omspan,a.nilai_omspan,a.nilai_pusat FROM DATA_OMSPAN25.rekon_potongan a
    ...
    try {
      const response = await axiosJWT.get(
        import.meta.env.VITE_REACT_APP_TKD_REFERENSI_25
          ? `${
              import.meta.env.VITE_REACT_APP_TKD_REFERENSI_25
            }${encryptedQuery}&thang=${thang}&kdpemda=${kdpemda}&kdkppn=${kodekppn}&bulan=${bulan}`
          : "",
  ...
  const getDataOmspan = async () => {
    ...
    try {
      const response = await axiosJWT.get(
        import.meta.env.VITE_REACT_APP_GENERATEDATA_REKON_OMSPAN_LIVE_25
          ? `${
              import.meta.env.VITE_REACT_APP_GENERATEDATA_REKON_OMSPAN_LIVE_25
