...
export default function DataKontrak(props) {
  const { axiosJWT, token, username, role, kdkanwil, kdkppn } = useContext(MyContext);
  ...
  const getData = async () => {
    ...
    let filterKppn = "";
    if (role === "3") {
      filterKppn = props.where + (props.where ? " AND " : "") + `a.kdkppn = '${kdkppn}'`;
    } else {
    ...
    const encodedQuery2 = encodeURIComponent(
      `SELECT a.id,a.kddept,b.nmdept,a.thang,a.kdunit,c.nmunit,a.kdsatker,i.nmsatker,a.kdlokasi,e.nmlokasi,a.kdkanwil,g.nmkanwil,a.kdkppn,h.nmkppn,a.uraian,a.tgpermohonan,
                ...
                LEFT JOIN dbref.t_satker_2024 i ON a.kdsatker=i.kdsatker  ${combinedFilter ? `WHERE ${combinedFilter}` : " "
    ...
    try {
      const response = await axiosJWT.get(
        import.meta.env.VITE_REACT_APP_LOCAL_TAYANGKONTRAK
          ? `${import.meta.env.VITE_REACT_APP_LOCAL_TAYANGKONTRAK
  ...
  const handleHapusDispKontrak = async (id, jumlah) => {
    const confirmText =
      jumlah > 0
