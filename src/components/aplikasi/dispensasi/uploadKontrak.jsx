...
const UploadKontrak = (props) => {
  ...
  const expectedColumns = ["tgkontrak", "nokontrak", "nilkontrak"]; // Header yang diharapkan
  const maxRows = 200;
  ...
  const getData = async () => {
    const encodedQuery = encodeURIComponent(
      `SELECT id,thang,kddept,kdunit,kdkanwil,kdlokasi,kdsatker,tgpermohonan,nopermohonan FROM  laporan_2023.dispensasi_kontrak WHERE id='${props.id}' GROUP BY id`
    );
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = (event) => {
      ...
      // Validasi format tanggal pada kolom tgkontrak
      const invalidRows = jsonData.filter((row) => !isValidDate(row.tgkontrak));
    ...
    <Container fluid>
      <Row>
        <Col md="12" className="my-1 mb-3 text-danger fw-bold">
          *) Catatan :<br /> 1. Excel berisi 3 kolom (tgkontrak | nokontrak |
          nilkontrak)
          <br />
