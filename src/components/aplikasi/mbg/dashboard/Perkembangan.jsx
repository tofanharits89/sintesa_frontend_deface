...
export default function Perkembangan() {
  ...
  const getKanwilOptions = () => {
    const allOptions = [
      ...
      { value: "ALL", label: "Semua Kanwil" },
      {
        value: "01 - DAERAH ISTIMEWA ACEH",
        label: "01 - DAERAH ISTIMEWA ACEH",
      },
      { value: "02 - SUMATERA UTARA", label: "02 - SUMATERA UTARA" },
      { value: "03 - SUMATERA BARAT", label: "03 - SUMATERA BARAT" },
      { value: "04 - RIAU", label: "04 - RIAU" },
    ...
    if (role === "2" && userKanwil) {
      ...
      return fullFormat
        ? [{ value: fullFormat, label: fullFormat }]
        : [{ value: userKanwil, label: userKanwil }];
    }
    return allOptions;
  };
  ...
  const [activeTriwulan, setActiveTriwulan] = useState(0); // State untuk triwulan aktif
  const [showRekapMenu, setShowRekapMenu] = useState(false); // State untuk slide down menu
  const [showRekapKanwil, setShowRekapKanwil] = useState(false);
  const [selectedTriwulan, setSelectedTriwulan] = useState(null);

  // Data rekapitulasi triwulan
  const rekapTriwulan = [
    {
      label: "Triwulan I",
      value: data.filter((row) => row.triwulan === "I").length,
      total: 34,
      color: "#10b981",
      textColor: "#065f46",
    },
    {
      label: "Triwulan II",
      value: data.filter((row) => row.triwulan === "II").length,
      total: 34,
      color: "#f59e0b",
      textColor: "#92400e",
    },
    {
      label: "Triwulan III",
      value: data.filter((row) => row.triwulan === "III").length,
      total: 34,
      color: "#3b82f6",
      textColor: "#1e40af",
    },
    {
      label: "Triwulan IV",
      value: data.filter((row) => row.triwulan === "IV").length,
      total: 34,
      color: "#8b5cf6",
      textColor: "#5b21b6",
    },
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await axiosJWT.get(
        import.meta.env.VITE_REACT_APP_LOCAL_NARASI + queryString,
        {
          headers: {
        ...
        const mappedData = responseData.map((row, index) => {
          return {
            ...
            perkembangan_makrokesra:
              row.perkembangan_makrokesra || "Belum ada data",
            perkembangan_harga_komoditas:
              row.perkembangan_harga_komoditas || "Belum ada data",
  ...
  const handleDownloadExcel = () => {
    try {
      ...
      // Download file
      const filterSuffix = `${
        kode_kanwil === "ALL" ? "All" : kode_kanwil.split(" - ")[0]
      }_${triwulan || "All"}_${tahun}`;
  ...
  const handleDownloadPDFDirect = async () => {
    ...
    try {
      ...
      pdf.setFontSize(10);
      const filterText = `Filter: Kanwil: ${
        kode_kanwil === "ALL" ? "Semua Kanwil" : kode_kanwil || "Semua Kanwil"
      }, Triwulan: ${triwulan || "Semua Triwulan"}, Tahun: ${tahun}`;
  ...
  return (
    ...
    >
      {/* Menu Rekapitulasi Upload Triwulan dengan Slide Down */}
      {["X", "1", "0"].includes(role) && (
        <div className="mb-4">
          {/* Button untuk toggle slide down */}
          <div
          ...
          >
            <div style={{ fontWeight: 700, fontSize: 18 }}>
              Total Upload <span style={{ color: "red" }}>Kanwil</span> DJPb
            </div>
            <div
              style={{
                transform: showRekapMenu ? "rotate(180deg)" : "rotate(0deg)",
          ...
          <div
            style={{
              maxHeight: showRekapMenu ? "500px" : "0px",
              overflow: "hidden",
              transition: "max-height 0.4s ease-in-out, padding 0.3s ease",
              padding: showRekapMenu ? "20px 0 0 0" : "0",
            }}
          >
            <Row className="g-3">
              {rekapTriwulan.map((tw, idx) => (
                <Col key={tw.label} xl={3} lg={4} md={6} sm={12}>
                  <Card
                    className={`h-100 shadow-sm ${
                      idx === activeTriwulan ? "border-primary" : ""
                    }`}
                    style={{
                      borderLeft: `6px solid ${tw.color}`,
                      cursor: "pointer",
                      transition: "all 0.2s ease",
                      transform: showRekapMenu
                        ? "translateY(0)"
                        : "translateY(-20px)",
                      opacity: showRekapMenu ? 1 : 0,
                      transitionDelay: showRekapMenu ? `${idx * 0.1}s` : "0s",
                    }}
                    onClick={() => {
                      setActiveTriwulan(idx);
                      setSelectedTriwulan(tw);
                      setShowRekapKanwil(true);
                    }}
                  >
                    <Card.Body className="text-center p-3">
                      ...
                      <div style={{ fontSize: 12, color: "#64748b" }}>
                        {Math.round((tw.value / tw.total) * 100)}% complete
                      </div>
                    </Card.Body>
                  </Card>
                </Col>
              ))}
              {/* Modal Rekap Kanwil per Triwulan - Compact */}
              <Modal
                show={showRekapKanwil}
                onHide={() => setShowRekapKanwil(false)}
                centered
                size="lg"
              >
                <Modal.Header
                ...
                >
                  ...
                  >
                    {KANWIL.filter(
                      (k) => k.value && k.value !== "ALL" && k.value !== ""
                    ).map((k) => {
                      const sudahSubmit = data.some((row) => {
                        // Ambil kode kanwil dari row.kanwil atau dari k.value
                        const rowKanwilCode =
                          row.kanwil?.split(" - ")[0] || row.kanwil;
                        const kanwilCode = k.value.split(" - ")[0];
                        const triwulanSelected =
                          selectedTriwulan?.label.split(" ")[1] || "";

                        return (
                          rowKanwilCode === kanwilCode &&
                          row.triwulan === triwulanSelected
                        );
                      });
                      return (
                        <div
                          ...
                          style={{
                            padding: "6px 8px",
                            borderRadius: 4,
                            background: sudahSubmit ? "#ecfdf5" : "#f8fafc",
                            border: sudahSubmit
                              ? "1px solid #10b981"
                              : "1px solid #e2e8f0",
                            display: "flex",
                        ...
                        >
                          <span
                            style={{
                              ...
                              height: 6,
                              borderRadius: "50%",
                              background: sudahSubmit ? "#10b981" : "#cbd5e1",
                              flexShrink: 0,
                            }}
                          ></span>
                          <span
                            style={{
                              color: sudahSubmit ? "#065f46" : "#64748b",
                              overflow: "hidden",
      ...
      >
        ...
        >
          {TRIWULANS.map((tw) => (
            <option value={tw.value} key={tw.value}>
              {tw.label}
            </option>
          ))}
        </select>
                  ...
                  >
                    <OverlayTrigger
                      ...
                      overlay={
                        ...
                        >
                          {row.perkembangan_harga_komoditas || "-"}
                        </Tooltip>
                      }
                    >
                      <span>
                        {sliceWords(row.perkembangan_harga_komoditas)}
                      </span>
                    </OverlayTrigger>
                  </td>
                  <td
      ...
      <Modal
        ...
        onHide={() => setDetailModal({ show: false, row: null })}
        centered
        size="xl"
      >
              ...
              >
                ...
                <tbody>
                  <tr>
                    ...
                    >
                      {detailModal.row.tahun || "-"}
                    </td>
                    <td style={{ padding: 8, wordBreak: "break-word" }}>
                      {detailModal.row.perkembangan_makrokesra || "-"}
                    </td>
                    <td style={{ padding: 8, wordBreak: "break-word" }}>
                      {detailModal.row.perkembangan_harga_komoditas || "-"}
                    </td>
                    <td style={{ padding: 8, wordBreak: "break-word" }}>
                      {detailModal.row.perkembangan_lainnya || "-"}
                    </td>
                    <td style={{ padding: 8, wordBreak: "break-word" }}>
                      {detailModal.row.permasalahan_isu || "-"}
                    </td>
                    <td style={{ padding: 8, wordBreak: "break-word" }}>
                      {detailModal.row.kesimpulan || "-"}
                    </td>
                    <td style={{ padding: 8, wordBreak: "break-word" }}>
                      {detailModal.row.rekomendasi || "-"}
                    </td>
                  </tr>
                </tbody>
              </table>
