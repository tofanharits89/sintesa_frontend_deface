...
const ExportPDF = ({ thang, dept, periode, cek, setCek }) => {
  ...
  const getDataIsu = async () => {
    ...
    const encodedQuery = encodeURIComponent(
      ...
      WHERE a.thang='${thang}' AND a.kddept='${dept}' AND a.periode='${periode}'
`
    );
  ...
  const getDataOutput = async () => {
    ...
    const encodedQuery = encodeURIComponent(
      `SELECT b.id as idedit,a.id_output,a.thang,a.kddept,a.periode,a.namaoutput,a.catatan,b.tahun,b.pagu,b.realisasi,b.persen FROM laporan_2023.output a
      LEFT JOIN laporan_2023.output_detail b ON a.id_output=b.id_output WHERE a.thang='${thang}' AND a.kddept='${dept}' AND a.periode='${periode}'
      `
    );
  ...
  const getDataTemuan = async () => {
    ...
    const encodedQuery = encodeURIComponent(
      `  SELECT a.id_temuan, a.id idtemuan, b.id iddetailtemuan, a.isu temuan,a.nilai,b.isu,a.id_temuan FROM laporan_2023.temuan_bpk a
      LEFT JOIN laporan_2023.temuan_bpk_detail b ON a.id_temuan=b.id_temuan WHERE a.thang='${thang}' AND a.kddept='${dept}' AND a.periode='${periode}'
      `
    );
  ...
  const getDataIkpa = async () => {
    ...
    const encodedQuery = encodeURIComponent(
      `SELECT a.id,a.thang,a.kddept,a.periode,a.username,a.nilaiikpa from laporan_2023.tren_ikpa a
      WHERE  a.kddept='${dept}' AND a.periode='${periode}' order by a.thang
      `
    );
  ...
  const generatePDF = () => {
    ...
    html2canvas(input, { scale: 3 }).then((canvas) => {
      ...
      pdf.save(`PROFILE_KL_${thang}_${dept}_${periode}.pdf`);
    });
    setLoadingpdf(false);
    setCek(false);
  };
      ...
      <div id="content-to-export" style={{ border: "0" }}>
        <Row>
          <Col xs={9} md={9} lg={9} xl={9}>
            ...
            <Row>
              <Col xs={4} md={4} lg={4} xl={4}>
                <Card className="card-kinerja-ikpa" onClick={handleShowTren}>
                  <Card.Body>
