...
const ChartUptup = ({ thang, periode, dept, prov, datauptup }) => {
  ...
  const getData = async () => {
    ...
    const encodedQuery = encodeURIComponent(
      ...
      WHERE
        a.thang ='${thang}' AND a.kddept='${dept}' GROUP BY thang,a.kddept,a.kdbkpk`
    );
    ...
    try {
      ...
      // const newFormattedSeries = response.data.result
      //   .map((item) => ({
      //     name: item.belanja,
      //     data: [
      //       Math.abs(parseFloat(item.tup)),
      //       Math.abs(parseFloat(item.ptup)),
      //     ],
      //   }))
      //   .map((item) => ({
      //     ...item,
  ...
  //   ...item,
  //   tup: parseFloat(item.tup),
  //   ptup: parseFloat(item.ptup),
  //   sisa: parseFloat(item.tup) - parseFloat(item.ptup),
  // }));

  // const series = updatedData.map((item) => ({
  //   data: [Math.abs(item.tup), Math.abs(item.ptup), Math.abs(item.sisa)],
  // }));
