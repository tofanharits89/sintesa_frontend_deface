  ...
  const getData = async () => {
    ...
    try {
      ...
      const encryptedQuery = Encrypt(
        `SELECT        a.kddept,        b.kdkanwil,        c.nmkanwil,        COUNT(DISTINCT CASE WHEN a.STATUS = 'true' THEN a.kdsatker END) AS sudah,        COUNT(DISTINCT(a.kdsatker)) - COUNT(DISTINCT CASE WHEN a.STATUS = 'true' THEN a.kdsatker END) AS belum,        COUNT(DISTINCT(a.kdsatker)) AS jumlahsatker,        ROUND((COUNT(DISTINCT CASE WHEN a.STATUS = 'true' THEN a.kdsatker END) / COUNT(DISTINCT(a.kdsatker))) * 100) AS persentase    FROM        spending_review.ref_satker_dipa_2025 a    LEFT JOIN        dbref.t_kppn_2025 b ON a.kdkppn = b.kdkppn    INNER JOIN        dbref.t_kanwil_2014 c ON b.kdkanwil = c.kdkanwil ${filterKanwil}    GROUP BY  c.kdkanwil`
      );
