...
const KertasKerja = ({ show, onHide, cek, kppn, kdpemda, thang, bulan }) => {
  ...
  const generateData = async () => {
    ...
    try {
      const response = await axiosJWT.get(
        import.meta.env.VITE_REACT_APP_LOCAL_GENERATEDATA
          ? `${
              import.meta.env.VITE_REACT_APP_LOCAL_GENERATEDATA
  ...
  const getData = async () => {
    ...
    const encodedQuery = encodeURIComponent(
      `SELECT thang,kdpemda,nmpemda,pagu,alokasi_bulan,upper(nmbulan) nmbulan,bulan,tunda,cabut,potongan,salur FROM tkd.rekap 
...
FROM tkd.detail_kmk_dau a 
LEFT JOIN tkd.ref_detail_potongan b ON a.jenis_kmk=b.jenis_pmk AND a.kriteria=b.kriteria AND a.kdakun=b.kdakun WHERE a.kdkabkota='${kdpemda}' AND a.bulan='${bulan}'`
    );
  ...
  const getDataRekap = async () => {
    ...
    const encodedQuery = encodeURIComponent(
      ...
      FROM tkd.ref_kmk_penundaan a 
      LEFT OUTER JOIN 
      (SELECT b.jenis,b.nmjenis FROM tkd.ref_kmk b) b ON a.jenis=b.jenis
      LEFT OUTER JOIN 
      (SELECT c.id,c.id_kriteria,c.nm_kriteria FROM tkd.ref_kmk_dau_kriteria c) c ON a.jenis=c.id AND a.kriteria=c.id_kriteria 
      LEFT OUTER JOIN
      (SELECT d.jenis,d.kriteria,d.kmk_cabut,d.kdkppn,d.kdpemda,d.kmk_tunda FROM tkd.ref_kmk_pencabutan d GROUP BY d.jenis,d.kriteria,d.kdkppn,d.kdpemda,d.kmk_tunda) d 
  ...
  const getDataRekapBulan = async () => {
    ...
    const encodedQuery = encodeURIComponent(
      ...
      potongan ,
      salur,
      kdkanwil from tkd.rekap WHERE kdpemda='${kdpemda}'`
    );
