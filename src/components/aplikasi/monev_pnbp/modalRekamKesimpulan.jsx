...
export default function RekamKesimpulan({ show, onHide }) {
    ...
    const getDataKesimpulan = async () => {
        ...
        const cleanedQuery = decodeURIComponent(encodedQuery).replace(/\n/g, " ").replace(/\s+/g, " ").trim();
        setSql(cleanedQuery);
        ...
        try {
            const response = await axiosJWT.get(
                `${import.meta.env.VITE_REACT_APP_LOCAL_TAYANG_KESIMPULAN_MONEVPNBP}${encryptedQuery}&limit=${limit}&page=${page}&user=${username}`,
                { headers: { Authorization: `Bearer ${token}` } }
            );
            ...
            handleHttpError(status, data?.error || "Terjadi Permasalahan Koneksi atau Server Backend");
        } finally {
        ...
        <Modal show={show} onHide={onHide} backdrop="static" size="lg">
            <Modal.Header closeButton>
                <Modal.Title>
                    ...
                    Rekam Gambaran Umum, Kesimpulan, dan Rekomendasi Pelaksanaan Monev PNBP
                </Modal.Title>
            </Modal.Header>
                        ...
                        <Row>
                            <Col md={4}>
                                <Form.Group>
                                    ...
                                    <Form.Select value={thang} onChange={(e) => setThang(e.target.value)}>
                            ...
                            <Col md={4}>
                                <Form.Group>
                                    ...
                                    <Form.Select value={triwulan} onChange={(e) => setTriwulan(e.target.value)}>
                            ...
                            <Col md={4}>
                                <Form.Group>
                                    ...
                                    <Kdkanwil value={kanwil} onChange={(value) => setKanwil(value)} />
                                </Form.Group>
                            </Col>
                        </Row>
                    </Card>

                    <Tabs defaultActiveKey="form-gambaran" className="mb-3 custom-tab" activeKey={activeTab} onSelect={(k) => setActiveTab(k)}>
                        <Tab eventKey="form-gambaran" title={<span className="text-dark">Gambaran Umum</span>}>
                        ...
                        <Tab eventKey="form-kesimpulan" title={<span className="text-dark">Kesimpulan</span>}>
                            <Card className="p-3 shadow-sm border-0">
                                <Form.Group className="mb-3">
                                    {/* <Form.Label>Uraian Upaya Harmonisasi</Form.Label> */}
                                    <Form.Control
                        ...
                        <Tab eventKey="form-rekomendasi" title={<span className="text-dark">Rekomendasi</span>}>
                            <Card className="p-3 shadow-sm border-0">
                                <Form.Group className="mb-3">
                                    {/* <Form.Label>Uraian Upaya Harmonisasi</Form.Label> */}
                                    <Form.Control
                        ...
                        <Tab eventKey="rekaman" title={<span className="text-dark">Hasil</span>}>
                            <Card className="p-3 shadow-sm border-0">
                                                ...
                                                {rekamanKesimpulan.length > 0 ? (
                                                    rekamanKesimpulan.map((item, index) => (
                                                        <tr key={item.id}>
                                                            ...
                                                            <td className="text-center">{item.thang}/{item.triwulan}</td>
                                                            <td className="text-center">({item.kdkanwil}) - {item.nmkanwil}</td>
                                                            <td className={`${styles.textJustify}`}>{item.gambaran_umum}</td>
                                                            <td className={`${styles.textJustify}`}>{item.kesimpulan}</td>
                                                            <td className={`${styles.textJustify}`}>{item.rekomendasi}</td>
                                                        </tr>
                                                    ))
                                                ) : (
                                                    <tr>
