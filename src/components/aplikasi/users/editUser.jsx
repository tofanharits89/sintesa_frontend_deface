...
const EditUser = ({ show, handleClose, username }) => {
  ...
  const getData = async () => {
    const encodedQuery = encodeURIComponent(
      "select * from v3.users where username='" + username + "'"
    );
    ...
    try {
      ...
      if (response.data) {
        ...
        setInitialValues({
          ...
          name: response.data[0].name, // Use default value if name is not available
          username: response.data[0].username,
          password: response.data[0].password,
          confirmPassword: response.data[0].password,
          role: response.data[0].role,
          deptlimit: response.data[0].dept_limit,
          telp: response.data[0].telp,
          email: response.data[0].email,
          file: "", // You can set this to an empty string or handle it separately
          kdkppn: response.data[0].kdkppn,
          kdkanwil: response.data[0].kdkanwil,
  ...
  const saveUser = async (values) => {
    ...
    try {
      await axiosJWT.patch(
        `${import.meta.env.VITE_REACT_APP_LOCAL_EDITUSER}/${initialValues.id}`,
        formData,
        {
          headers: {
      ...
      setLoading(false);
      NotifikasiSukses("User Berhasil Diupdate");
    } catch (error) {
      const { status, data } = error.response || {};
      handleHttpError(
        status,
  ...
  return (
    ...
    >
      <Modal.Header closeButton>
        <Modal.Title>
          <h5 className="text-dark">Edit User </h5>
        </Modal.Title>
      </Modal.Header>
