...
const KinerjaBansosBanper = ({ thang, periode, dept, kdkanwil, kdkppn }) => {
  ...
  let triwulan = Math.ceil(periode / 3);
  ...
  return (
    <Container fluid>
      <div className="my-2 text-center fw-bold">
        Kinerja Bansos Banper Triwulan {triwulanRomawi} <br /> TA {thang}
      </div>
        ...
        <div className="table-responsive">
          <Table striped bordered hover responsive className="text-center mt-4">
            ...
            <tbody>
              {data.length > 0 ? (
                data.map((item, index) => {
                  const nilaiBarangMiliar = item.nilai_barang / 1e9 || 0;
                  const realBarangMiliar = item.real_barang / 1e9 || 0;
                  const nilaiModalMiliar = item.nilai_modal / 1e9 || 0;
                  const realModalMiliar = item.real_modal / 1e9 || 0;
                  ...
                  return (
                    <tr key={index}>
                      <td>{item.nmsatker}</td>
                      <td>{numeral(item.jml_barang).format("0,0")}</td>
...
export default KinerjaBansosBanper;
