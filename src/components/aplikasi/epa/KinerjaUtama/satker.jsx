...
export const Satker = ({ periode, kdkanwil, kdkppn }) => {
  ...
  const getData = async () => {
    ...
    const encodedQuery = encodeURIComponent(
      `SELECT a.kdsatker,a.nmsatker,sum(a.blokir) blokir,sum(a.pagu) pagu,SUM(${realColumns}) AS realisasi 
      FROM digitalisasi_epa.pagu_real_utama a 
      WHERE a.thang = ${dataEpa.year} AND a.kddept = ${dataEpa.kddept} 
      GROUP BY a.kdsatker,a.nmsatker`
    );
  ...
  const totalPagu = dataEselon.reduce(
    (sum, row) => sum + Number(row.pagu || 0),
    0
  );
  const totalRealisasi = dataEselon.reduce(
    (sum, row) => sum + Number(row.realisasi || 0),
    0
  );
  const totalBlokir = dataEselon.reduce(
  ...
  return (
    <Container fluid>
      <h5 className="my-3">
        Kinerja Satker Utama Tahun {dataEpa.year} (dalam rupiah)
      </h5>
      <Card>
