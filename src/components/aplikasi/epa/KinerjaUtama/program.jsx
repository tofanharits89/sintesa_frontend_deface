...
export const Program = ({ periode, kdkanwil, kdkppn }) => {
  ...
  const getData = async () => {
    ...
    const encodedQuery = encodeURIComponent(
      `SELECT a.kdprogram,a.nmprogram,sum(a.blokir) blokir,sum(a.pagu) pagu,SUM(${realColumns}) AS realisasi 
      FROM digitalisasi_epa.pagu_real_utama a 
      WHERE a.thang = ${dataEpa.year} AND a.kddept = ${dataEpa.kddept} 
      GROUP BY a.kdprogram,a.nmprogram`
    );
  ...
  return (
    <Container fluid>
      <h5 className="my-3">
        Kinerja Program Utama Tahun {dataEpa.year} (dalam rupiah)
      </h5>
      <Card>
        <Card.Body
          style={{
              ...
              <Table striped bordered hover responsive className="mt-3">
                ...
                <tfoot>
                  <tr style={{ background: "#f8f9fa", fontWeight: "bold" }}>
                    ...
                    <td className="text-end">{totalPersen}</td>
