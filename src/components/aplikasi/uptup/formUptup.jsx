...
import { getSQLUptup } from "./SQLUptup";
import Thang from "./ThangUptup";
import GenerateCSV from "../CSV/generateCSV";
import JenisLaporanUptup from "./JenisLaporanUptup";
import BulanUptup from "./bulanUptup";
import HasilQueryUptup from "./hasilQueryUptup";
import PembulatanUptup from "./PembulatanUptup";
...
const InquiryUptup = () => {
  ...
  const handleCutoffcek = () => {
    let realisasi = `,round(sum(rupiah)/${pembulatan},0) as rupiah`;
    let from = "monev" + thang + ".pa_up_" + thang + "_outstanding  a";
  ...
  const generateSql = () => {
    ...
    getSQLUptup(queryParams);
    const query = getSQLUptup(queryParams);
    setSql(query);
  };
  // HANDLE KDDEPT
  const [deptkondisi, setDeptkondisi] = useState("");
    ...
    <>
      ...
      {export2 && (
        <GenerateCSV
          query3={sql}
          status={handleStatus}
          namafile={`v3_CSV_UPTUP_${moment().format("DDMMYY-HHmmss")}`}
        />
      )}
      {loadingExcell && (
        <GenerateExcel
          ...
          namafile={`v3_EXCELL_UPTUP_JENIS_${jenlap}_${moment().format(
            "DDMMYY-HHmmss"
          )}.xlsx`}
        />
      )}
      ...
      <SaveUserData userData={telp} menu="uptup" />
    </>
  );
};
