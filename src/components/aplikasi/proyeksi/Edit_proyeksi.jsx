...
const Edit_proyeksi = ({ show, onHide, setRefresh, id }) => {
  ...
  useEffect(() => {
    if (data.length > 0) {
      const proyeksiData = data[0];

      setKeperluan(proyeksiData.keperluan);
      setPeriode(proyeksiData.periode);
      setTahun(proyeksiData.thang);
      setCekKppn(`${proyeksiData.kdkppn}-${proyeksiData.kdsatker}`);
      setJenis(proyeksiData.jenis_tkd);
      setKdsatker(proyeksiData.kdsatker);
      setJanuari(proyeksiData.jan);
      setFebruari(proyeksiData.feb);
      setMaret(proyeksiData.mar);
      setApril(proyeksiData.apr);
      setMei(proyeksiData.mei);
      setJuni(proyeksiData.jun);
      setJuli(proyeksiData.jul);
      setAgustus(proyeksiData.ags);
      setSeptember(proyeksiData.sep);
      setOktober(proyeksiData.okt);
      setNovember(proyeksiData.nov);
      setDesember(proyeksiData.des);
      setUraian(proyeksiData.keterangan);
    }
  }, [data]);
  ...
  const getData = async () => {
    ...
    const encodedQuery = encodeURIComponent(
      `SELECT a.id,a.keperluan,a.thang,a.periode,a.kdkppn,a.kdsatker,a.jenis_tkd,a.jan,a.feb,a.mar,a.apr,a.mei,a.jun,a.jul,a.ags,a.sep,a.okt,a.nov,a.des,a.keterangan FROM
      tkd.proyeksi a where a.id=${id}
      `
    );
  ...
  const handleSubmitdata = async (values, { setSubmitting }) => {
    ...
    try {
      await axiosJWT.patch(
        import.meta.env.VITE_REACT_APP_LOCAL_EDITPROYEKSI,
        values,
        {
          headers: {
                    ...
                    <Row>
                      <Col sm={12} md={6} lg={6} xl={6} className="mt-2 mb-3">
                        <Form.Group controlId="inputName5">
                          ...
                          <Form.Select
                            value={tahun} // Gunakan state sebagai nilai untuk memilih opsi
                            name="tahun"
                            onChange={(e) => {
                          ...
                          >
                            ...
                            {/* <option value="2023" selected={tahun === "2023"}>
                              2023
                            </option> */}
                            <option value="2024" selected={tahun === "2024"}>
                              2024
                            </option>
                            <option value="2025" selected={tahun === "2025"}>
                              2025
                            </option>
                          </Form.Select>
                      ...
                      <Col sm={12} md={6} lg={6} xl={6} className={`mt-2 mb-3`}>
                        <Form.Group controlId="kppn">
                          ...
                          <CekKppn
                            ...
                            component="div"
                            className="text-danger"
                            onChange={(value) => handleCekKppn(value)}
                          />
                        </Form.Group>
                      </Col>
                    </Row>
