...
const HasilQueryBelwilBulanan = (props) => {
  const { showModalBulanan, closeModalBulanan } = props;
  const { axiosJWT, token, username } = useContext(MyContext);
  const [loading, setLoading] = useState(false);
  const [sql] = useState(props.queryBulanan);
  ...
  const getData = async () => {
    ...
    try {
      const response = await axiosJWT.get(
        import.meta.env.VITE_REACT_APP_LOCAL_INQUIRY_GETDATA_BELWIL_BULANAN
          ? `${
              import.meta.env.VITE_REACT_APP_LOCAL_INQUIRY_GETDATA_BELWIL_BULANAN
  ...
  data.forEach((row) => {
    for (
      let cellIndex = jumlahKolom - 12;
      cellIndex < jumlahKolom;
      cellIndex++
    ) {
      columnTotals[cellIndex - (jumlahKolom - 12)] += Number(
  ...
  return (
    <>
      <Modal
        onHide={tutupModalBulanan}
        show={showModalBulanan}
        backdrop="static"
        keyboard={false}
        size="xl"
        animation={false}
...
export default HasilQueryBelwilBulanan;
