...
const HasilQueryAkumulasi = (props) => {
  const { showModalAkumulasi, closeModalAkumulasi } = props;
  const { axiosJWT, token, username } = useContext(MyContext);
  const [loading, setLoading] = useState(false);
  const [sql] = useState(props.queryAkumulasi);
  ...
  const [pagu_dipa, setpagu_dipa] = useState(0);
  ...
  const getData = async () => {
    ...
    try {
      const response = await axiosJWT.get(
        import.meta.env.VITE_REACT_APP_LOCAL_INQUIRYAKUMULASI
          ? `${
              import.meta.env.VITE_REACT_APP_LOCAL_INQUIRYAKUMULASI
      ...
      setpagu_dipa(response.data.pagu_dipa);
      setblokir(response.data.blokir);
  ...
  const columnTotals = new Array(14).fill(0);
  ...
  data.forEach((row) => {
    for (
      ...
      columnTotals[cellIndex - (jumlahKolom - 14)] += Number(
  ...
  return (
    <>
      <Modal
        onHide={tutupModalAkumulasi}
        show={showModalAkumulasi}
        backdrop="static"
        keyboard={false}
        size="xl"
        animation={false}
      ...
      >
        ...
        <Modal.Body>
          ...
          {data.length > 0 && (
            <>
              <div className="pagination justify-content-between mt-2 ">
                <span>
...
export default HasilQueryAkumulasi;
