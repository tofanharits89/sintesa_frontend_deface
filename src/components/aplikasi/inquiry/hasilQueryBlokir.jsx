...
const HasilQueryBlokir = (props) => {
  const navigate = useNavigate();

  const { showModalBlokir, closeModalBlokir } = props;
  const { axiosJWT, token, username } = useContext(MyContext);
  const [loading, setLoading] = useState(false);
  const [sql] = useState(props.queryBlokir);
  ...
  const getData = async () => {
    ...
    try {
      const response = await axiosJWT.get(
        import.meta.env.VITE_REACT_APP_LOCAL_INQUIRYBLOKIR
          ? `${import.meta.env.VITE_REACT_APP_LOCAL_INQUIRYBLOKIR
...
export default HasilQueryBlokir;
