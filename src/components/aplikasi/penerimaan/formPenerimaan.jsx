...
import { getSQLPenerimaan } from "./SQLPenerimaan";
import Thang from "./ThangPenerimaan";
import GenerateCSV from "../CSV/generateCSV";
import JenisLaporanPenerimaan from "./JenisLaporanPenerimaan";
import BulanPenerimaan from "./bulanPenerimaan";
import HasilQueryPenerimaan from "./hasilQueryPenerimaan";
import PembulatanPenerimaan from "./PembulatanPenerimaan";
...
import SumberRadio from "../inquiry/radio/sumberRadio";
...
const InquiryPenerimaan = () => {
  ...
  const [cutoff, setCutoff] = useState(currentMonth);
  ...
  const handleCutoffcek = () => {
    let realisasi = `,a.tgpos,a.nodok,round(sum(rupiah)/${pembulatan},0) as rupiah`;
    let from = "monev" + thang + ".pa_pnbp_" + thang + " a";
  ...
  //   setAkun(akun);
  // };
  const [kdsumber, setKdSumber] = useState(false);
  const [sumberradio, setSumberradio] = useState("1");
  const [sumber, setSumber] = useState("XX");
  const handleKdSumber = (sumber) => {
    setKdSumber(sumber);
    setSumber(sumber);
  };
  const getSwitchSumber = (kdsumber) => {
    if (kdsumber) {
      setSumber("00");
    } else {
      setSumber("XX");
    }
    setKdSumber(kdsumber);
  };
  const handleRadioSumber = (sumberRadio) => {
    setSumberradio(sumberRadio);
  };
  ...
  const generateSql = () => {
    ...
    getSQLPenerimaan(queryParams);
    const query = getSQLPenerimaan(queryParams);
    setSql(query);
  };
  // HANDLE KDDEPT
  const [deptkondisi, setDeptkondisi] = useState("");
    ...
    <>
        ...
        <div className="pagetitle">
          ...
          <nav>
            <ol className="breadcrumb">
              ...
              <li className="breadcrumb-item active ">PNBP</li>
            </ol>
          </nav>
        </div>
                ...
                <Card.Body>
                  <div className="bagian-query">
                    <div className="custom-content">
                      ...
                      />
                      <JenisLaporanPenerimaan
                        jenlap={jenlap}
                        onChange={handleJenlap}
                      />
                      <PembulatanPenerimaan
                        value={pembulatan}
                ...
                <Card.Body>
                  <div className="bagian-query">
                    ...
                    <Row>
                      ...
                      <Col xs={6} md={6} lg={6} xl={3}>
                        <pilihanData.SwitchSumber onChange={getSwitchSumber} />
                      </Col>
                    </Row>
                  </div>
                </Card.Body>
            ...
            <Card.Body>
              ...
              {kdsumber && (
                <>
                  <Row className="mt-2">
                    <Col xs={4} sm={4} md={4}>
                      <span className="middle  ">Sumber PNBP</span>
                    </Col>

                    <Col xs={4} sm={4} md={4}>
                      <Sumber onChange={handleKdSumber} />
                    </Col>
                    <Col xs={4} sm={4} md={4}>
                      <SumberRadio
                        sumberRadio={handleRadioSumber}
                        selectedValue={sumberradio}
                      />
                    </Col>
                  </Row>
                </>
      ...
      {export2 && (
        <GenerateCSV
          query3={sql}
          status={handleStatus}
          namafile={`v3_CSV_Penerimaan_PNBP_${moment().format(
            "DDMMYY-HHmmss"
          )}`}
        />
      )}
      {loadingExcell && (
        <GenerateExcel
          ...
          namafile={`v3_EXCELL_Penerimaan_PNBP_JENIS_${jenlap}_${moment().format(
            "DDMMYY-HHmmss"
          )}.xlsx`}
        />
      )}
      ...
      <SaveUserData userData={telp} menu="penerimaan" />
    </>
  );
};

export default InquiryPenerimaan;
