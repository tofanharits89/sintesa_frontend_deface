export const getSQLPenerimaan = (queryParams) => {
  const {
    thang,
    jenlap,
    role,
  ...
  function buatQuery() {
    ...
    if (cutoff !== "XXX") {
      ...
      whereConditions.push(
        cutoff && cutoff !== "000" ? `month(a.tgpos) <= '${cutoff}'` : ``
      );
    }
    ...
    // console.log(nilaiawalkdkabkota);
    ...
    if (sumber !== "XX") {
      kolom.push(
        "(CASE WHEN a.je_source = 'Payables' THEN '1' WHEN a.je_source = 'Receivables' THEN '2' END) AS je_source"
      );
      group.push("a.je_source");
    }

    if (sumberradio === "2") {
      kolom.push("dd.nmsumber");
      query += ` LEFT JOIN dbref.t_sumber_pnp dd on a.je_source=dd.je_source`;
    }
    if (sumberradio === "3") {
      kolom.pop("dd.nmsumber");
      kolom.push("dd.nmsumber");
      query += ` LEFT JOIN dbref.t_sumber_pnp dd on a.je_source=dd.je_source`;
    }
    if (sumberradio === "4") {
    ...
    if (sumber !== "XX") {
      whereConditions.push(
        sumber && sumber !== "00" ? `a.je_source = '${sumber}'` : ``
      );
    }
