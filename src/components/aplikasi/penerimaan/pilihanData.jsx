...
export const SwitchSumber = ({ sumber, onChange }) => {
  const [switchSumber, setswitchSumber] = useState(false);
  const [value, setValue] = useState("");

  useEffect(() => {
    setValue(sumber);
  }, [sumber]);
  const handleSwitchSumber = () => {
    const newSwitchSumber = !switchSumber;
    setswitchSumber(!switchSumber);
    onChange(newSwitchSumber);
    setswitchSumber(!switchSumber);
  };
  return (
    <>
      <Form.Check
        ...
        id="custom-switch"
        label="Status Sumber"
        checked={switchSumber}
        onChange={handleSwitchSumber}
      />
    </>
  );
};
