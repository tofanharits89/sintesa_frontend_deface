...
const HasilQuery = (props) => {
  ...
  const getData = async () => {
    ...
    try {
      const response = await axiosJWT.get(
        import.meta.env.VITE_REACT_APP_LOCAL_INQUIRY_GETDATA_TEMATIK_APBD
          ? `${import.meta.env.VITE_REACT_APP_LOCAL_INQUIRY_GETDATA_TEMATIK_APBD
      ...
      setData(response.data.result.filter((item) => item.kdprov !== "XX"));
  ...
  const columnTotalSums = new Array(12).fill(0);
  ...
  data.forEach((row) => {
    for (
      ...
      columnTotalSums[cellIndex - (jumlahKolom - 12)] += Number(
    ...
    try {
      const response = await axiosJWT.get(
        import.meta.env.VITE_REACT_APP_LOCAL_TGUPDATE
          ? `${import.meta.env.VITE_REACT_APP_LOCAL_TGUPDATE}?thang=${props.thang
          }`
          : "",
        {
          headers: {
