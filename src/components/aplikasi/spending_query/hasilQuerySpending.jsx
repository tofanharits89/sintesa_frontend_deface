...
const HasilQuerySpending = (props) => {
  const { showModal, closeModal, jenlap, jenis_efisiensi } = props;
  ...
  const getData = async () => {
    ...
    try {
      const response = await axiosJWT.get(
        import.meta.env.VITE_REACT_APP_LOCAL_INQUIRY_GETDATA_SPENDING
          ? `${import.meta.env.VITE_REACT_APP_LOCAL_INQUIRY_GETDATA_SPENDING
          ...
          <>
            ...
            <div className={fit}>
              ...
              >
                ...
                <tbody>
                  {data.map((row, index) => {
                    const batasKolomNumerik =
                      jenis_efisiensi !== "1" ? jumlahKolom - 5 : jumlahKolom - 4;
                    // console.log(batasKolomNumerik);
                    return (
                      <tr key={index}>
                        <td className="text-tengah">
                          {index + 1 + page * limit}
                        ...
                        {Object.values(row).map((cell, index) => (
                          <React.Fragment key={index}>
                            {index > batasKolomNumerik && index !== jumlahKolom - 1 ? (
                              <td className="text-kanan">
                                {numeral(cell).format("0,0")}
                              </td>
                            ) : (
...
export default HasilQuerySpending;
