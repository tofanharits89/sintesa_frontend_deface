...
export const SwitchsOutput = ({ onChange }) => {
...
export const SwitchAlasan = ({ jenlap, onChange }) => {
  const [switchAlasan, setswitchAlasan] = useState(false);
  const [value, setValue] = useState("");

  useEffect(() => {
    setValue(jenlap);
  }, [jenlap]);
  const handleSwitchAlasan = () => {
    const newSwitchAlasan = !switchAlasan;
    setswitchAlasan(!switchAlasan);
    onChange(newSwitchAlasan);
    setswitchAlasan(!switchAlasan);
  };
  return (
    <>
      <Form.Check
        ...
        id="custom-switch"
        label="Alasan"
        checked={switchAlasan}
        onChange={handleSwitchAlasan}
        disabled={jenlap === "6"}
      />
    </>
  );
