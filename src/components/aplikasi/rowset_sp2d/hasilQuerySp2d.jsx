...
import { Modal, Spinner, Form, Tooltip, OverlayTrigger } from "react-bootstrap";
...
const HasilQuerySp2d = (props) => {
  ...
  const getData = async () => {
    ...
    try {
      const response = await axiosJWT.get(
        import.meta.env.VITE_REACT_APP_LOCAL_INQUIRY_GETDATA_SP2D
          ? `${
              import.meta.env.VITE_REACT_APP_LOCAL_INQUIRY_GETDATA_SP2D
  ...
  const columnTotals = new Array(2).fill(0);

  data.forEach((row) => {
    for (
      let cellIndex = jumlahKolom - 2;
      cellIndex < jumlahKolom;
      cellIndex++
    ) {
      columnTotals[cellIndex - (jumlahKolom - 2)] += Number(
                        ...
                        ([columnName, cell], cellIndex) => (
                          <React.Fragment key={cellIndex}>
                            {columnName === "URAIAN" ? (
                              ...
                              <td className="text-kanan fw-bold">
