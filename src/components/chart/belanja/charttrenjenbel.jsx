...
const TrenJenbel = (props) => {
  ...
  var options = {
    ...
    labels: data.map((item) => item.kdjenbel),
    dataLabels: {
      enabled: true,
  ...
  const series = data.map((item) =>
    parseFloat(numeral(item.persentase_pagu).format("0,0.0"))
  );
